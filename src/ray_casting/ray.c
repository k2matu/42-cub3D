...
/*   Updated: 2024/10/21 16:10:49 by hzibari          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double check_y_hits(t_struct *game)
{
	...
	while(i <= game->ray->map_hight) // maybe need a limit untill map high or wight incase ray is staright then it will never find wall; 
	{
...
double check_x_hits(t_struct *game)
{
	...
	while(i <= game->ray->map_weight) // maybe same as the top one
	{
		if (check_wall_hit(game, x_inter, y_inter))
			break;
		y_inter += y_step;
...
void raycasting(t_struct *game)
{
	...
	while (ray_id < S_W)
	{
		...
		mlx_draw_line(game->img, game->ray->pixel_pos_x, game->ray->pixel_pos_y, x_end, y_end, 0xFFFF00);
		//render_wall();
